...
- **2026-08-28** — Rebuilt landing page hero (`src/components/LandingHero.tsx`) to match exact interaction/animation specification:
  - **Layout**: Strict 3-region single-viewport (`100vh`/`100dvh`) flex layout (`.page`), padded `clamp(16px, 2.4vh, 28px) clamp(14px, 3vw, 32px)`, with header (shrink 0), centered hero (flex 1), and stats footer (shrink 0).
  - **Header**: Circular logo button (`clamp(40px, 4.4vw, 46px)`, white bg, hover scale `1.04`), white pill nav (`clamp(44px, 5.2vw, 48px)` height, max-width 430px, 3-dot active indicator via `::after` box-shadow), dark pill CTA (`#28282a`, `#c8c8c8` text, hover `#323234`), animated slideDown entrance (`0.7s cubic-bezier(0.22, 1, 0.36, 1)` from `opacity 0 translateY(-18px)`).
  - **Fonts**: Added Inter via `next/font/google` in `layout.tsx` for UI elements, paired with Google Font `DotGothic16` for the 2-line uppercase dot-matrix headline ("Track Your Academic Journey").
  - **Trust Row**: Overlapping department ring avatars (`--trust-size: clamp(36px, 4.5vw, 42px)`, `#28282a` outer ring with `1px rgba(255,255,255,0.4)` border, `calc(var(--trust-size) * -0.42)` overlap, hover lift) ending with "Trusted by IIT Bhubaneswar Students".
  - **Hero CTA**: White pill CTA ("Calculate Now") with soft white glow, Inter 600, entrance delay 0.4s.
  - **Stats Footer**: 4 metrics (7 Departments, 10 Point Grading Scale, 8 Semesters Tracked, 1,250+ Students Using It) with Lucide React icons, staggered `easeOutCubic` count-up animation (`0.5s`/`0.58s`/`0.66s`/`0.74s` at 0.25 threshold).
  - **Mobile**: Circular 48x48px burger button, full-screen `rgba(0,0,0,0.62)` blur overlay, 28px radius white sheet menu with staggered links.
  - **Motion & Accessibility**: `.anim` shared entrance (`0.85s cubic-bezier(0.22, 1, 0.36, 1)` fade+translateY+blur), and `prefers-reduced-motion: reduce` overrides in `globals.css`.
