...
- **2026-08-28** — Configured Google Font `Oswald` via `next/font/google` in `src/app/layout.tsx` (`--font-oswald`) and added `.font-oswald` utility class in `globals.css`. Updated landing page hero headline in `src/components/LandingHero.tsx` to use `Oswald` bold display font (`font-oswald`) for strong, modern headline aesthetics.
- **2026-08-28** — Updated landing page hero headline typography in `src/components/LandingHero.tsx`: switched two-line headline ("Track Your Academic Journey") from dot-matrix/pixel font (`font-pixel` / `DotGothic16`) to modern sans-serif (`GeistSans`, `font-extrabold` / weight 800) for visual harmony with the VisionOS glassmorphic design system while retaining solid white color, two-line layout, responsive letter-spacing, and staggered line entrance animations.
